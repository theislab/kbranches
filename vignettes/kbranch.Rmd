...
Then, the data can be plotted using cluster assignment `clust$cluster` as color:

![Demonstration of kbranch.global for K=3 branches.](figs/kbranch_global.png)
...
```{r kbranch.local.filter, eval=FALSE}
...
plot(input_dat, pch=21, col = tips$cluster + 1, bg = tips$cluster + 1,
     main='tip regions')



#plot the branching_region(s), in this example we only have one branching region
plot(input_dat, pch = 21, col = branch_reg$cluster + 1, bg = branch_reg$cluster + 1,
     main = 'branching regions')

```

![Three tip and one branching regions were identified.](figs/identified_regions.png)
